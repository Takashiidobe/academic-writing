---
title: "Aliasing in C"
bibliography: "src/aliasing-in-c.bib"
---

## Aliasing in C

You can alias functions in C using `__attribute__(weak,
alias($new_name))` or using inline asm.
...
<details>
<summary>The generated asm code</summary>
```sh
$ gcc -S alias.c -O0 -o-
```
...
The 2.3 version is the default, but there's a 2.2.5 version as well.
We can explicitly use an older symbol:
...
given power. We can then export it to the same symbol.
By default callers will get `power@@v2`, but they can also call the
older version too.
...
And run it, which returns 25 (5 * 5), so we've called the older version.
